...
def baricentric(object point,object element):
    p = point
...
def semi_lagrange2(object nodes,object elements,np.ndarray[np.double_t, ndim=1] vx,np.ndarray[np.double_t, ndim=1] vy,np.ndarray[np.double_t, ndim=1] T,double dt,np.ndarray[np.int_t, ndim=2] IENbound, object boundary_list):
    
    cdef int count, e, vizinho
    vxd = np.zeros( (len(vx)),dtype='float' )
    vyd = np.zeros( (len(vx)),dtype='float' )
    ...
    for point in nodes:
        ...
        if point.ID in boundary_list and not point.out_flow:
            vxd[point.ID] = vx[point.ID]
            vyd[point.ID] = vy[point.ID]
