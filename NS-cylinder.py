...
msh = meshio.read('cilindro_gustavo.msh')
X = msh.points[:,0]
Y = msh.points[:,1]
...
cylinder_bound = np.array((),dtype='int')
...
for i in range (len(IENboundElem)):
    ...
    elif IENboundElem[i] == 'cylinder_bound':
        if IENbound[i,0] not in cylinder_bound:
            cylinder_bound= np.append(cylinder_bound, IENbound[i,0])
        if IENbound[i,1] not in cylinder_bound:
            cylinder_bound = np.append(cylinder_bound, IENbound[i,1])
...
# vetor do lado direito ( f=M*Q/(rho*cv) )

vx=np.zeros( (npoints),dtype='float' )
vy=np.zeros( (npoints),dtype='float' )
p = np.ones( (npoints_p),dtype='float' )
...
x_part[:,0] = np.random.rand(nparticles)
x_part[:,1] = 0.1 + 0.8*np.random.rand(nparticles)

d_part = 0.01*np.ones( (x_part.shape[0]),dtype='float' )
rho_part = 7.0*np.ones( (x_part.shape[0]),dtype='float' )
...
if not os.path.isdir('C:\VTK\\NS-cylinder'):
    os.mkdir('C:\VTK\\NS-cylinder')
for i in range(len(t)):
    print(i)
    
    #Semi-lagrangiano
    # vxd, vyd = calc_vd(IEN,IENbound,X,Y,vx,vy,npoints_p,dt)

    vxd, vyd, Td = semi_lagrange2(Node.node_list,Element.elem_list,vx,vy,T,dt,IENbound)
    
    Matriz_vx = np.block([[(1.0/dt)*M + (1.0/Re)*K,np.zeros( (npoints,npoints),dtype='float' ), Gx]])
    vetor_vx = np.dot((1.0/dt)*M,vxd)
    
    Matriz_vy = np.block([[np.zeros( (npoints,npoints),dtype='float' ),(1.0/dt)*M + (1.0/Re)*K, Gy]])
    vetor_vy = np.dot((1.0/dt)*M,vyd)
    ...
    x_part = particleCloud.solve(dt,nLoop,rho,Re,Fr)
    ...
    'C:\VTK\\NS-cylinder\sol-'+str(i)+'.vtk',
    msh.points,
    msh.cells,
    ...
    if x_part.shape[0] != 0:
        x_p = x_part[:,0].copy()
        y_p = x_part[:,1].copy()
        pointsToVTK("C:\VTK\\NS-cylinder\sol_particles"+str(i),x_p,y_p,0.1*np.ones((x_part.shape[0]), dtype='float'))
