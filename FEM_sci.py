...
class FEM:
    ...
    def set_matrices(cls,mesh,fluid,dt,BC,IC,SolidProp,porous = False, turb = False):
        ...
        if len(cls.mesh.FSI) > 0:
            ...
            SolidFEM.set_parameters(cls.solidMesh, cls.BC_solid, cls.IC, float(SolidProp['h']), float(SolidProp['E']), dt, float(SolidProp['rho']), float(SolidProp['nu']), float(SolidProp['gamma']), float(SolidProp['beta']), [0,-1], 1.0/cls.Ga**2)
